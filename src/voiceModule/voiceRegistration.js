...
export const VoiceRegistration = (user) => {
  ...
  const upload = async () => {
    ...
    let executionResult = await Axios({
      ...
      stateMachineArn:
        "arn:aws:states:us-east-1:432795727339:stateMachine:voice-registration-step-function"
    }
    }).catch((error) => console.error(error));
    ...
    if (jsonResponseOfExecution.data.status !== "RUNNING" || jsonResponseOfExecution.data.statu ==="FAILED"){
      break;
    }
  }
  // if (jsonResponseOfExecution.data.body.statusCode != 200) {
  //   if (parseFloat(res.data.body.confidence_lvl) <= 0.82) {
  //     document.getElementById("submit").innerHTML = "Re-submit";
  //     div.innerHTML = "This is someone else, try again!<br/>";
  //   } else {
  //     document.getElementById("submit").innerHTML = "Re-submit";
  //     div.innerHTML =
  //       "Congrats you have successfully verified your voice and have matched your Voice Profile with a Confidence Score of " +
  //       res.data.body.confidence_lvl;
  //   }
  // }
  document.getElementById("submit").innerHTML = "Re-submit";
  ...
  div.innerHTML = (jsonResponseOfExecution.data.output);
      ...
      <div className="columns is-centered is-vcentered">
        <div className="column is-one-fifth ">
          ...
          <div className="NewQuoteGenerator container">
            <p className="button is-fullwidth column is-4000">{index}</p>
          </div>
          <br></br>
          <br></br>
